import { getNumberOfDays, getNth, getDaysInMonth, getMonthName } from './helpers'

// #region Functions that setup and trigger other functions 
export const setupDiary = (element:any) => {
  ...
  Array.from(element.querySelectorAll('tbody td')).forEach((arrayElement:any , index) => {
...
function populateDays(element:any,month:number,year:number){
...
function poulateEvents(element){
  ...
  Array.from(element.querySelectorAll('.events li[data-start-date]')).forEach((arrayElement:any , index) => {
